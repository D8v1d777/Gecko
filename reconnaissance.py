...
class ReconEngine:
    ...
    async def _dns_bruteforce(self, domain: str) -> List[str]:
        ...
        common_subdomains = [
            ...
            'ns2', 'cpanel', 'whm', 'autodiscover', 'autoconfig', 'stage', 'staging', 'dev',
            'development', 'test', 'api', 'admin', 'portal', 'dashboard', 'app', 'mobile',
            'blog', 'forum', 'shop', 'store', 'cdn', 'static', 'assets', 'img', 'images'
        ]
...
class APIFuzzer:
    ...
    async def _test_mass_assignment(self, url: str) -> Optional[Dict]:
        ...
        try:
            async with self.session.post(url, json=dangerous_fields, timeout=10) as resp:
