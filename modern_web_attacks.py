        ...
        for depth in [10, 20, 50, 100]:
            ...
            try:
                async with self.session.post(
                    url, json={"query": nested_query}, timeout=15
                ) as resp:
                    if resp.status == 200:
                        return depth
        ...
        try:
            ...
            for claim in privilege_claims:
                if claim in decoded:
                    ...
                    modified[claim] = (
                        "admin" if isinstance(decoded[claim], str) else True
                    )
...
class SSTIDetector:
            ...
            for template_engine, payloads in self.payloads.items():
                for payload in payloads:
                    result = await self._test_payload(
                        url, param, payload, template_engine
                    )
                    if result:
    ...
    async def _test_payload(
        self, url: str, param: str, payload: str, engine: str
    ) -> Optional[Dict]:
        """Test SSTI payload."""
