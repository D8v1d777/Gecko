...
class OAuthExploiter:
    ...
    async def _test_redirect_uri(self, endpoint: str) -> Optional[Dict]:
        ...
        for redirect in test_redirects:
            ...
            try:
                async with self.session.get(test_url, allow_redirects=False, timeout=10) as resp:
...
class BusinessLogicTester:
    ...
    async def _test_race_condition(self, url: str) -> Optional[Dict]:
        ...
        if 'POST' not in str(url) and not any(k in url.lower() for k in ['checkout', 'purchase', 'redeem', 'claim']):
            return None
    ...
    async def _test_price_manipulation(self, url: str) -> Optional[Dict]:
        ...
        price_params = [k for k in params if any(p in k.lower() for p in ['price', 'amount', 'total', 'cost'])]
    ...
    async def _test_negative_quantity(self, url: str) -> Optional[Dict]:
        ...
        qty_params = [k for k in params if any(q in k.lower() for q in ['qty', 'quantity', 'count', 'amount'])]
    ...
    async def _test_xss(self, url: str, param: str) -> Optional[Dict]:
        ...
        for payload in payloads:
            ...
            try:
                async with self.session.get(test_url, timeout=10) as resp:
                    content = await resp.text()
                    if payload in content and 'Content-Type' not in resp.headers or 'text/html' in resp.headers.get('Content-Type', ''):
                        return {
...
class AuthBypass:
    ...
    async def _test_password_reset_poisoning(self, url: str) -> Optional[Dict]:
        ...
        if not any(keyword in url.lower() for keyword in ['reset', 'forgot', 'password']):
            return None
        ...
        try:
            async with self.session.post(url, headers=headers, data={'email': 'test@test.com'}, timeout=10) as resp:
                if resp.status == 200:
                    return {
