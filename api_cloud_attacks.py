...
class SSRFProber:
    ...
    def _find_url_parameters(self, url: str, content: str) -> List[str]:
        ...
        url_params = ['url', 'uri', 'path', 'dest', 'redirect', 'link', 'src', 
                      'source', 'callback', 'return', 'page', 'site', 'target']
        
        return [p for p in params if any(keyword in p.lower() for keyword in url_params)]
    ...
    def _verify_ssrf(self, target: str, content: str, response) -> bool:
        ...
        if '169.254.169.254' in target:
            indicators = ['ami-id', 'instance-id', 'security-credentials', 'AccessKeyId']
    ...
    def _determine_severity(self, target: str) -> str:
        ...
        if any(cloud in target for cloud in ['169.254.169.254', 'metadata.google.internal']):
            return 'CRITICAL'
        if 'file://' in target:
            return 'HIGH'
        return 'MEDIUM'
...
<lolz>&lol4;</lolz>'''
        ...
        try:
            headers = {'Content-Type': 'application/xml'}
            async with self.session.post(url, data=payload, headers=headers, timeout=30) as resp:
        ...
        try:
            ...
            async with self.session.get(url, headers=headers, timeout=10) as resp:
                ...
                if acao == origin and acac == 'true':
                    severity = 'CRITICAL' if origin in ['null', 'https://evil.com'] else 'HIGH'
                    return {
