...
class DatabaseManager:
    ...
    def store_url(self, url: str, status_code: int = 0, content_type: str = '', content_length: int = 0):
        """Store visited URL."""
        c = self.conn.cursor()
        c.execute('''
    ...
    def get_statistics(self) -> Dict:
        ...
        c.execute('SELECT severity, COUNT(*) as count FROM findings WHERE false_positive = 0 GROUP BY severity')
        for row in c.fetchall():
            stats['by_severity'][row['severity']] = row['count']
            stats['total_findings'] += row['count']

        c.execute('SELECT type, COUNT(*) as count FROM findings WHERE false_positive = 0 GROUP BY type ORDER BY count DESC LIMIT 20')
        for row in c.fetchall():
