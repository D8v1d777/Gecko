...
OWASP_MAP = {
    ...
    'CWE-78': 'A03:2021 - Injection', 'CWE-94': 'A03:2021 - Injection',
    'CWE-918': 'A10:2021 - SSRF', 'CWE-611': 'A05:2021 - Security Misconfiguration',
    'CWE-327': 'A02:2021 - Cryptographic Failures', 'CWE-200': 'A01:2021 - Broken Access Control',
    'CWE-284': 'A01:2021 - Broken Access Control', 'CWE-639': 'A01:2021 - Broken Access Control',
    'CWE-16': 'A05:2021 - Security Misconfiguration', 'CWE-295': 'A07:2021 - Identification Failures',
    'CWE-22': 'A01:2021 - Broken Access Control', 'CWE-362': 'A04:2021 - Insecure Design',
    'CWE-942': 'A05:2021 - Security Misconfiguration', 'CWE-346': 'A07:2021 - Identification Failures',
    'CWE-601': 'A01:2021 - Broken Access Control', 'CWE-269': 'A01:2021 - Broken Access Control',
    'CWE-915': 'A04:2021 - Insecure Design', 'CWE-319': 'A02:2021 - Cryptographic Failures',
}

COMPLIANCE_FLAGS = {
    'CWE-89': ['PCI-DSS 6.5.1', 'GDPR Art.32'], 'CWE-79': ['PCI-DSS 6.5.7'],
    'CWE-200': ['GDPR Art.5', 'CCPA 1798.150'], 'CWE-319': ['PCI-DSS 4.1', 'GDPR Art.32'],
    'CWE-295': ['PCI-DSS 4.1'], 'CWE-284': ['PCI-DSS 7.1', 'GDPR Art.25'],
    'CWE-22': ['PCI-DSS 6.5.8'], 'CWE-611': ['PCI-DSS 6.5.1'],
}
...
class ReportGenerator:
    ...
    async def generate(self, fmt, target, findings):
        ...
        generators = {
            'json': self._gen_json, 'html': self._gen_html,
            'markdown': self._gen_markdown, 'pdf': self._gen_pdf,
        }
    ...
    async def _gen_json(self, target, findings):
        path = self.output_dir / f"gecko_report_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
        report = {
            'meta': {'tool': 'Gecko Apocalypse v10.0', 'target': target,
                     'timestamp': datetime.utcnow().isoformat(), 'stats': self.stats},
    ...
    async def _gen_html(self, target, findings):
        path = self.output_dir / f"gecko_report_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.html"
...
<p style="text-align:center;color:#475569;margin-top:40px">Generated by Gecko Apocalypse Engine v10.0</p>
    ...
    async def _gen_markdown(self, target, findings):
        path = self.output_dir / f"gecko_report_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.md"
        by_sev = {}
        for f in findings:
            s = f.get('severity','INFO')
        ...
        for i, f in enumerate(findings, 1):
            ...
            md += f"- **URL:** {f.get('url','')}\n"
            if f.get('parameter'): md += f"- **Parameter:** {f['parameter']}\n"
            if f.get('cvss_score'): md += f"- **CVSS:** {f['cvss_score']}\n"
            if f.get('cwe'): md += f"- **CWE:** {f['cwe']}\n"
            if f.get('owasp'): md += f"- **OWASP:** {f['owasp']}\n"
            md += f"- **Evidence:** {f.get('evidence','')}\n"
            if f.get('remediation'): md += f"- **Remediation:** {f['remediation']}\n"
            if f.get('compliance_flags'): md += f"- **Compliance:** {', '.join(f['compliance_flags'])}\n"
            md += "\n---\n\n"
    ...
    async def _gen_pdf(self, target, findings):
        ...
        try:
            ...
            path = self.output_dir / f"gecko_report_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.pdf"
            pdf = FPDF()
            pdf.add_page()
            pdf.set_font('Helvetica', 'B', 20)
            pdf.cell(0, 15, 'Gecko Apocalypse Security Report', new_x="LMARGIN", new_y="NEXT", align='C')
            pdf.set_font('Helvetica', '', 12)
            pdf.cell(0, 10, f'Target: {target}', new_x="LMARGIN", new_y="NEXT")
            pdf.cell(0, 10, f'Date: {datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")}', new_x="LMARGIN", new_y="NEXT")
            pdf.cell(0, 10, f'Total Findings: {len(findings)}', new_x="LMARGIN", new_y="NEXT")
            pdf.ln(10)
            ...
            for i, f in enumerate(findings, 1):
                ...
                pdf.cell(0, 8, f"{i}. [{f.get('severity','')}] {f.get('type','')}", new_x="LMARGIN", new_y="NEXT")
                pdf.set_font('Helvetica', '', 9)
                pdf.cell(0, 6, f"URL: {f.get('url','')}", new_x="LMARGIN", new_y="NEXT")
                if f.get('evidence'):
                    pdf.multi_cell(0, 5, f"Evidence: {f['evidence'][:200]}")
                if f.get('remediation'):
                    pdf.cell(0, 6, f"Fix: {f['remediation']}", new_x="LMARGIN", new_y="NEXT")
                pdf.ln(3)
                if pdf.get_y() > 260:
                    pdf.add_page()
            pdf.output(str(path))
    ...
    def _exec_summary(self, findings):
        ...
        risk = 'CRITICAL' if crit > 0 else 'HIGH' if high > 0 else 'MEDIUM' if by_sev.get('MEDIUM',0) > 0 else 'LOW'
        return (f"The security assessment identified {total} findings. "
