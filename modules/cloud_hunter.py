...
class CloudHunter:
    ...
    async def _azure_blob(self, url, content):
        ...
        for b in set(blobs)[:5]:
            try:
                async with self.session.get(
                    f"https://{b}.blob.core.windows.net/?comp=list",
                    timeout=aiohttp.ClientTimeout(total=10),
                ) as r:
                    if r.status == 200:
                        return {
                            "type": "Azure Blob Public",
                            "severity": "HIGH",
                            "url": f"https://{b}.blob.core.windows.net/",
                            "evidence": "Public blob storage",
                            "remediation": "Restrict blob access",
                            "cwe": "CWE-284",
                        }
            except:
                pass
    ...
    async def _gcs(self, url, content):
        ...
        for b in set(buckets)[:5]:
            try:
                async with self.session.get(
                    f"https://storage.googleapis.com/{b}",
                    timeout=aiohttp.ClientTimeout(total=10),
                ) as r:
                    if r.status == 200:
                        return {
                            "type": "GCS Bucket Public",
                            "severity": "HIGH",
                            "url": f"https://storage.googleapis.com/{b}",
                            "evidence": "Public GCS bucket",
                            "cwe": "CWE-284",
                        }
            except:
                pass
    ...
    async def _takeover(self, url, content):
        fps = {
            "heroku": "no-such-app",
            "github": "There isn't a GitHub Pages",
                    ...
                    if r.status == 200:
                        ...
                        if "kind" in c and "apiVersion" in c:
                            return {
                                ...
                                "severity": "CRITICAL",
                                "url": f"{base}{p}",
                                "evidence": "K8s API accessible",
                                "remediation": "Restrict K8s API access",
                                "cwe": "CWE-284",
                            }
            except:
                pass
                ...
                async with self.session.get(
                    ...
                    if r.status == 200:
                        return {
                            ...
                            "severity": "CRITICAL",
                            "url": f"{base}{p}",
                            "evidence": "Docker registry accessible",
                            "cwe": "CWE-284",
                        }
            except:
                pass
    ...
    async def _serverless(self, url, content):
        ...
        for p in patterns:
            if re.search(p, content):
                return {
                    "type": "Serverless Function Detected",
                    "severity": "INFO",
                    "url": url,
                    "evidence": f"Serverless pattern: {p}",
                    "description": "Serverless infrastructure detected",
                }
    ...
    async def _cdn_poison(self, url, content):
        ...
        try:
            async with self.session.get(
                url, headers=poison_headers, timeout=aiohttp.ClientTimeout(total=10)
            ) as r:
                c = await r.text(errors="ignore")
                if "evil.com" in c:
                    return {
                        "type": "CDN Cache Poisoning",
                        "severity": "HIGH",
                        "url": url,
                        "evidence": "Unkeyed header reflected in cached response",
                        "remediation": "Normalize cache keys, strip unkeyed headers",
                        "cwe": "CWE-444",
                    }
        except:
            pass
