    ...
    async def scan(self, url, content, headers, response):
        ...
        for stype, pattern in self.patterns.items():
            for m in set(re.findall(pattern, content, re.IGNORECASE)):
                ...
                if ent > 3.0 or stype in ['Private Key','Database URL','Firebase URL']:
                    sev = 'CRITICAL' if stype in ['AWS Access Key','Private Key','Database URL','Stripe Live Key'] else \
                          'HIGH' if stype in ['GitHub Token','SendGrid Key','Slack Token','Google API Key'] else 'MEDIUM'
                    findings.append({'type':f'Exposed Secret: {stype}','severity':sev,'url':url,
                                    'evidence':(val[:50]+'...' if len(val)>50 else val),
                                    'description':f'{stype} exposed','remediation':'Remove and rotate credentials',
                                    'cwe':'CWE-200','owasp':'A01:2021'})
        return findings

    def _entropy(self, data):
        if not data: return 0
        return -sum(data.count(c)/len(data) * math.log2(data.count(c)/len(data)) for c in set(data))
