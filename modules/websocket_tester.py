...
class WebSocketTester:
    ...
    async def _test_origin(self, ws_url):
        ...
        try:
            h = {'Upgrade':'websocket','Connection':'Upgrade','Sec-WebSocket-Key':'dGhlIHNhbXBsZSBub25jZQ==',
                 'Sec-WebSocket-Version':'13','Origin':'https://evil.com'}
            async with self.session.get(http_url, headers=h, timeout=aiohttp.ClientTimeout(total=10)) as r:
                if r.status == 101:
                    return {'type':'WebSocket Origin Bypass','severity':'HIGH','url':ws_url,
                            'evidence':'WebSocket accepted from arbitrary origin',
                            'description':'Cross-Site WebSocket Hijacking possible',
                            'remediation':'Validate Origin header','cwe':'CWE-346','owasp':'A07:2021'}
        except: pass
    ...
    async def _test_unauth(self, ws_url):
        ...
        try:
            ...
            async with self.session.get(http_url, headers=h, timeout=aiohttp.ClientTimeout(total=10)) as r:
                if r.status == 101:
                    return {'type':'WebSocket No Authentication','severity':'MEDIUM','url':ws_url,
                            'evidence':'WebSocket connection without auth','cwe':'CWE-306'}
        except: pass
