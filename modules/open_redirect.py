...
class OpenRedirect:
    ...
    async def scan(self, url, content, headers, response):
        ...
        payloads = [
            ...
            "https:evil.com", "http://legit.com.evil.com",
            "http://evil.com#legit.com", "http://evil.com?legit.com",
            "/%09/evil.com", "/%5c/evil.com"
        ]
        
        for param in params:
            # Heuristic to save requests: only test params that look like destinations
            if not any(k in param.lower() for k in ['url', 'redirect', 'next', 'return', 'dest', 'goto']):
                continue
                
            for p in payloads:
                test = self._inject(url, param, p)
                try:
                    async with self.session.get(test, allow_redirects=False, timeout=aiohttp.ClientTimeout(total=10)) as r:
                        loc = r.headers.get('Location', '')
                        if 'evil.com' in loc:
                            findings.append({'type':'Open Redirect','severity':'MEDIUM','url':url,
                                    'parameter':param,'payload':p,'evidence':f'Server redirected to: {loc}',
