            ...
            for p in payloads:
                ...
                try:
                    async with self.session.get(test, timeout=aiohttp.ClientTimeout(total=10)) as r:
                        ...
                        if any(e in c for e in ['LDAPException', 'com.sun.jndi.ldap', 'Search: Bad search filter']):
                            findings.append({'type':'LDAP Injection','severity':'HIGH','url':url,
                                    'parameter':param,'payload':p,'evidence':'LDAP error returned in response',
