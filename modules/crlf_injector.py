                ...
                try:
                    async with self.session.get(test, allow_redirects=False, timeout=aiohttp.ClientTimeout(total=10)) as r:
                        if 'crlf=injection' in r.headers.get('Set-Cookie', ''):
                            findings.append({'type':'CRLF Injection (HTTP Response Splitting)','severity':'HIGH','url':url,
                                    'parameter':param,'payload':p,'evidence':'Injected Set-Cookie header found in response',
                                    'remediation':'Sanitize carriage return and line feed characters in user input','cwe':'CWE-113','owasp':'A03:2021'})
                            break
                except: pass
        return findings
