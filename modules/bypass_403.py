...
class Bypass403:
    ...
    async def scan(self, url, content, headers, response):
        ...
        for h in spoof_headers:
            try:
                async with self.session.get(url, headers=h, timeout=aiohttp.ClientTimeout(total=10)) as r:
                    if r.status == 200:
                        findings.append({'type':'403/401 Bypass','severity':'HIGH','url':url,
                                'evidence':f'Bypassed restriction using header: {h}',
                                'remediation':'Do not trust client-controlled headers for access control','cwe':'CWE-284'})
            except: pass
            
        # Path manipulation
        path_manipulations = [
            f"{path}/", f"{path}%2e", f"{path}/.", f"//.{path}", f"{path}%20"
        ]
        ...
        for p in path_manipulations:
            ...
            try:
                async with self.session.get(test_url, timeout=aiohttp.ClientTimeout(total=10)) as r:
                    if r.status == 200:
                        findings.append({'type':'403/401 Bypass (Path)','severity':'HIGH','url':url,
