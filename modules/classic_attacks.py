...
class ClassicAttacks:
        ...
        for p in payloads:
            ...
            try:
                async with self.session.get(
                    test, timeout=aiohttp.ClientTimeout(total=15)
                ) as r:
                    el = time.time() - t
    ...
    async def _lfi(self, url, param):
        ...
        for p in payloads:
            try:
                async with self.session.get(
                    self._inject(url, param, p), timeout=aiohttp.ClientTimeout(total=10)
                ) as r:
                    c = await r.text(errors="ignore")
    ...
    async def _cmdi(self, url, param):
        ...
        for p in payloads:
            ...
            try:
                async with self.session.get(
                    self._inject(url, param, p), timeout=aiohttp.ClientTimeout(total=15)
                ) as r:
                    if time.time() - t > 4.5:
                        return {
                            ...
                            "parameter": param,
                            "payload": p,
                            "evidence": f"Delayed {time.time()-t:.1f}s",
                            "cwe": "CWE-78",
                            "owasp": "A03:2021",
                        }
            except:
    ...
    async def _open_redirect(self, url, param):
        if not any(
            k in param.lower()
            for k in ["url", "redirect", "next", "return", "goto", "dest", "continue"]
        ):
            return
        for target in ["https://evil.com", "//evil.com", "https://evil.com@legit.com"]:
            try:
                async with self.session.get(
                    self._inject(url, param, target),
