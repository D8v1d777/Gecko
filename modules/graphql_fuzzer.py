...
class GraphQLFuzzer:
    ...
    async def scan(self, url, content, headers, response):
        findings = []
        if not self._is_gql(url, content, headers):
            return findings
    ...
    def _is_gql(self, url, content, headers):
        return any(
            [
                ...
                "__schema" in content,
                '"data":' in content and '"errors":' in content,
            ]
        )
    ...
    async def _introspection(self, url):
        ...
        try:
            async with self.session.post(
                url, json=q, timeout=aiohttp.ClientTimeout(total=10)
            ) as r:
                ...
                if "__schema" in str(d):
                    return {
                        ...
                        "severity": "MEDIUM",
                        "url": url,
                        "evidence": json.dumps(d)[:500],
                        "description": "GraphQL introspection exposes full schema",
    ...
    async def _depth(self, url):
        for d in [10, 25, 50, 100]:
            ...
            try:
                async with self.session.post(
                    url, json={"query": q}, timeout=aiohttp.ClientTimeout(total=15)
                ) as r:
                    if r.status == 200:
                        return {
                            ...
                            "severity": "HIGH",
                            "url": url,
                            "evidence": f"Server accepted depth {d}",
                            "cwe": "CWE-400",
                            "owasp": "A04:2021",
                ...
                async with self.session.post(
                    ...
                    if r.status == 200:
                        return {
                            ...
                            "severity": "MEDIUM",
                            "url": url,
                            "evidence": f"Processed {sz} batched queries",
            ...
            async with self.session.post(
                ...
                if el > 5:
                    return {
                        ...
                        "severity": "HIGH",
                        "url": url,
                        "evidence": f"{el:.1f}s with 500 aliases",
                    }
        except asyncio.TimeoutError:
            return {
                "type": "GraphQL Alias Resource Exhaustion",
                "severity": "HIGH",
                "url": url,
                "evidence": "Timeout with 500 aliases",
            }
        except:
            pass
