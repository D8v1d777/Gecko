                ...
                try:
                    async with self.session.get(test, timeout=aiohttp.ClientTimeout(total=10)) as r:
                        ...
                        if 'uid=' in c or '<DIR>' in c or '.shtml' in c:
                            findings.append({'type':'Server-Side Includes (SSI) Injection','severity':'CRITICAL','url':url,
                                    'parameter':param,'payload':p,'evidence':'Command output or SSI variable found in response',
