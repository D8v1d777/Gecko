        ...
        try:
            async with self.session.get(test_url, timeout=aiohttp.ClientTimeout(total=10)) as r:
                ...
                if self._verify(payload, c):
                    return {'type':'Server-Side Template Injection','severity':'CRITICAL','url':url,
                            'parameter':param,'payload':payload,'evidence':f'Engine: {engine}',
                            'description':f'SSTI in {engine}','remediation':'Never render user input in templates',
                            'cwe':'CWE-94','owasp':'A03:2021 - Injection'}
        except: pass

    def _verify(self, payload, resp):
        if '7*7' in payload and '49' in resp: return True
        if '7*"7"' in payload and '7777777' in resp: return True
        if 'config.items()' in payload and 'SECRET_KEY' in resp: return True
        if '__subclasses__' in payload and '<class' in resp: return True
        if 'smarty.version' in payload and re.search(r'\d+\.\d+', resp): return True
        return False
