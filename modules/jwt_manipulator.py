...
class JWTManipulator:
    ...
    def _extract(self, content, headers):
        ...
        if "Bearer" in auth:
            tokens.add(auth.split("Bearer ")[-1].strip())
    ...
    async def _alg_confusion(self, url, token):
        try:
            ...
            async with self.session.get(
                url,
                headers={"Authorization": f"Bearer {mod}"},
                ...
                if r.status == 200:
                    return {
                        ...
                        "severity": "CRITICAL",
                        "url": url,
                        "evidence": "Server accepted token with none algorithm",
                        "cwe": "CWE-327",
                        "owasp": "A02:2021",
            ...
            async with self.session.get(
                ...
                if r.status == 200:
                    return {
                        ...
                        "severity": "CRITICAL",
                        "url": url,
                        "evidence": "Server accepted unsigned JWT",
                        "cwe": "CWE-345",
                    }
        except:
            pass
            ...
            for s in self.weak_secrets:
                try:
                    ...
                    return {
                        ...
                        "severity": "CRITICAL",
                        "url": url,
                        "evidence": f"JWT secret cracked: '{s}'",
                        "cwe": "CWE-521",
                    }
                    ...
                    async with self.session.get(
                        ...
                        if r.status == 200:
                            return {
                                ...
                                "severity": "HIGH",
                                "url": url,
                                "evidence": "Server accepts expired JWT tokens",
                                "cwe": "CWE-613",
                            }
        except:
            pass
        ...
        try:
            ...
            for claim in [
                ...
                if claim in dec:
                    ...
                    p = (
                        base64.urlsafe_b64encode(json.dumps(mod).encode())
                        .decode()
                        .rstrip("=")
                    )
                    async with self.session.get(
                        url,
                        headers={"Authorization": f"Bearer {h}.{p}."},
                        timeout=aiohttp.ClientTimeout(total=10),
                    ) as r:
                        if r.status == 200:
                            findings.append(
                                {
                                    ...
                                    "severity": "CRITICAL",
                                    "url": url,
                                    "evidence": f"Modified claim '{claim}'",
                                    "cwe": "CWE-269",
                                }
                            )
        except:
