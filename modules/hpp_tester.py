            ...
            try:
                async with self.session.get(test_url, timeout=aiohttp.ClientTimeout(total=10)) as r:
                    ...
                    if 'gecko_hpp_test' in c:
                        findings.append({'type':'HTTP Parameter Pollution','severity':'MEDIUM','url':url,
                                'parameter':param,'evidence':'Application reflects the second (polluted) parameter',
                                'remediation':'Strictly validate and extract the first or last parameter consistently','cwe':'CWE-235'})
                        break
            except: pass
