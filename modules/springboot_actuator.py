                ...
                async with self.session.get(
                    ...
                    if r.status == 200:
                        ...
                        if (
                            ...
                            sev = (
                                "CRITICAL" if "env" in a or "heapdump" in a else "HIGH"
                            )
                            findings.append(
                                {
                                    "type": "Spring Boot Actuator Exposed",
                                    "severity": sev,
                                    "url": test_url,
                                    "evidence": f"Actuator endpoint {a} is accessible",
                                    "remediation": "Restrict access to actuator endpoints using Spring Security",
                                    "cwe": "CWE-538",
                                }
                            )
            except:
